...
function InvoicesPage({ selectedProfile }) {
    ...
    useEffect(() => {
        ...
        if (selectedProfile.profile_type !== 'business') {
            setError('Invoices can only be added to business profiles.');
            setLoading(false);
            return;
        }
        ...
        const fetchInitialData = async () => {
            try {
                ...
                const clientsRes = await axios.get(`${API_URL}/clients/profile/${selectedProfile.profile_id}`, authHeaders);
                ...
                await fetchInvoices(selectedProfile.profile_id, token);
            } catch (err) {
    ...
    const renderInvoiceForm = () => {
        if (!selectedProfile || selectedProfile.profile_type !== 'business') {
            return <p className="text-negative text-sm text-center mb-4">Please select a Business Profile to create invoices.</p>;
        }
        if (clients.length === 0) {
    ...
    return (
        <div className="w-full max-w-5xl mx-auto p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col gap-4 p-6 bg-card-light dark:bg-card-dark rounded-xl border border-border-light dark:border-border-dark">
                    <h3 className="text-text-light dark:text-text-dark text-lg font-semibold">Invoices for {selectedProfile?.profile_name || 'Selected Profile'}</h3>
                    {renderInvoiceList()}
                </div>
            </div>
        </div>
